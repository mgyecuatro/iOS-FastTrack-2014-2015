...
let divide_closure = { (numerator n : Int, denominator d: Int) -> Double? in
   if (d == 0) {
      return nil
   } else {
      let y = Double(n) / Double(d)
...
divide_closure(numerator: 12, denominator: 3)
...
let y  = { (numerator n : Int, denominator d: Int) -> Double? in
      if (d == 0) {
         return nil
      } else {
         let y = Double(n) / Double(d)
...
let cuboidVolumeClosure = { (width w : Double, height h: Double, depth d: Double) -> Double in
   return w*h*d
}
cuboidVolumeClosure(width: 2.0,height: 3.0, depth: 4.0)
...
closureArray.append({ (width w : Double, height h: Double, depth d: Double) in return w*h*d} )
...
closureArray.append({ (width w : Double, height h: Double, depth d: Double) in w*h*d} )
...
//: Solution
let vol1 = { (width w:Double, height h:Double, depth d:Double) -> Double in
   return w * h * d
}
//: Test
vol(width: 2.0, height: 3.0, depth: 4.0) - vol1(width: 2.0, height: 3.0, depth: 4.0)
